...
3. 대화를 통해 Night Owl, Creator, Artist, Explorer의 네 DNA 트랙을 성장시킵니다.
4. 권장 데모에서는 Night Owl DNA 30에 도달해 전용 외형을 가진 Night Owl 진화를 달성합니다.
...
첫 번째 입력으로 기억이 생성되고, 권장 입력의 `night` 주제가 누적되면 Night Owl 진화가 발생합니다. 진화 후 세 번째 입력은 현실 퀘스트로 변환됩니다. 모델의 주제 분류에 따라 점수 증가량의 세부 조합은 달라질 수 있습니다.
...
- Momo의 Night Owl 진화 달성
- `Quest Keeper` 능력 해금
- 현실 퀘스트 한 개 생성 및 완료

완료 뒤에도 DNA, 기억, Bond는 브라우저에 계속 저장됩니다. 다시 시작하려면 우측 상단 초기화 버튼을 사용합니다.

Character DNA 엔진은 네 성장 트랙을 계산하지만, 이 제출 빌드에서 전용 진화 외형까지 제공하는 형태는 **Night Owl 1종**입니다. Creator, Artist, Explorer는 DNA 점수와 성격 라벨로 표현됩니다.
...
웹 플레이 기본 링크는 Cloudflare Worker를 통해 Gemini 3.6 Flash Live AI를 사용합니다. 네트워크 장애 시에는 자동으로 Offline Fallback으로 전환되고 다음 메시지에서 재연결합니다.
...
### Gemini 3.6 Flash 연동 실행(Gemini API)

1. `.env.example`을 참고해 로컬 전용 `.env.local`을 만듭니다.
2. Google AI API 키는 `GEMINI_API_KEY`라는 서버 전용 환경 변수로만 설정합니다.
3. `VITE_API_BASE_URL`은 로컬 프록시 주소를 가리키게 합니다.
4. 첫 번째 터미널에서 `npm run ai:server`를 실행합니다.
5. 두 번째 터미널에서 `npm run dev -- --host 127.0.0.1 --port 4627`을 실행합니다.
...
구현된 기능은 캐릭터 시작 화면, 채팅, 중요한 기억 저장, 네 Character DNA 성장 트랙, Night Owl 전용 외형 진화, 능력 해금, 현실 퀘스트 생성·완료, 보상, 이벤트 애니메이션, 로컬 진행 저장과 AI 장애 시 Demo AI 전환입니다.
