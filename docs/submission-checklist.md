...
플레이 영상: 40초 실제 플레이 MP4 및 YouTube 링크 반영 대기
...
- [ ] 로컬 또는 공개 서버 프록시를 통해 Gemma 4 26B A4B IT(`gemma-4-26b-a4b-it`) Structured Output 실호출을 검증했다.
- [x] Live AI 연결 실패 시 9초 안에 Demo Safe 모드로 전환되는 경로를 구현했다.
- [x] 배포 번들·Git 추적 파일에 API 키 패턴이 없음을 검색했다.
- [x] `.env.local`과 테스트 산출물은 Git에서 제외했다.
- [x] 게임 상태는 새로고침 후 유지되며 초기화 버튼으로 삭제할 수 있다.
- [ ] 현재 Markdown에서 게임 소개 PDF를 다시 생성하고 제목, 개요, 목표, 조작, 종료 조건, 실행법, 플레이·소스·영상 링크를 확인한다.
- [ ] 현재 Markdown에서 AI 기술 PDF를 다시 생성하고 모델명, 전체 System Instruction, JSON Schema, 점수 규칙, 폴백, 도구 활용, 에셋·오픈소스 출처를 확인한다.
- [ ] 두 최종 PDF의 A4 페이지에서 한글·표·코드·스크린샷·URL이 정상 렌더링되는지 확인한다.
...
- [ ] 40초 실제 플레이 MP4를 `submission/`에 추가하고 재생을 확인한다.
...
40초 MP4 파일과 YouTube 링크는 최종 영상이 준비된 뒤 반영합니다. 공개 기본 URL의 Live AI 성공이 확인되기 전까지 Live 완료로 표시하지 않습니다. `?demo=1`은 네트워크 장애에 대비해 전체 흐름을 완주할 수 있는 강제 Demo Safe 경로입니다.
